...
public class JobController {
    ...
    @GetMapping
    public ResponseEntity<Page<JobDto>> getAllJobs(@RequestParam(defaultValue = "0") @Min(0) int page, @RequestParam(defaultValue = "10") @Min(1) @Max(100) int size) {
        return ResponseEntity.ok(jobService.getAllJobs(page, size));
    }

    @GetMapping("/created-by/{userId}")
    public ResponseEntity<List<JobDto>> getJobsByCreator(@PathVariable Long userId) {
        return ResponseEntity.ok(jobService.getJobsByCreator(userId));
    }
