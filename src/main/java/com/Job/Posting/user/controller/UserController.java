...
public class UserController {
    ...
    @PostMapping
    public ResponseEntity<UserDto> addNewUser(@RequestBody @Valid AddUserRequestDto dto) {
        return ResponseEntity.status(HttpStatus.CREATED).body(userService.addNewUser(dto));
    }

    @PutMapping("/{id}")
    public ResponseEntity<UserDto> updateUser(@RequestBody @Valid AddUserRequestDto dto,
                                              @PathVariable Long id) {
        return ResponseEntity.status(HttpStatus.ACCEPTED).body(userService.updateUser(dto, id));
    }

    @PatchMapping("/{id}")
    public ResponseEntity<UserDto> updateUserValue(@RequestBody Map<String, Object> updates,
                                                   @PathVariable Long id) {
    ...
    public ResponseEntity<UserDto> addSkillToUser(@PathVariable Long userId,
    ...
    public ResponseEntity<Void> deleteSkillFromUser(@PathVariable Long userId,
                                                    @PathVariable Long skillId) {
        userService.deleteSkillFromUser(userId, skillId);
        return ResponseEntity.noContent().build();
    }

    @PostMapping("/oauth-profile/{appUserId}")
    public ResponseEntity<UserDto> createOAuthProfile(@PathVariable Long appUserId,
                                                      @RequestBody @Valid OAuthProfileRequestDto dto) {
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(userService.createOAuthProfile(appUserId, dto));
    }
}
